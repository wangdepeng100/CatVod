...
class Kan360Spider extends Spider {

    constructor() {
        super();
        this.siteUrl = 'https://www.360kan.com';
    }

    getName() { return "👩‍👩‍👧|三六影视" }

    getAppName() { return "三六影视" }

    getJSName() { return "kan360" }
    ...
    async category(inReq, _outResp) {
        ...
        // &cat=言情&year=2024&area=内地&act=
        // let url = this.siteUrl + '/' + tid + '/list?rank=ranklatest' + '&pageno=' + pg;
        let url = 'https://api.web.360kan.com/v1/filter/list?catid=' + tid +'&rank=ranklatest&cat=&year=&area=&act=&size=35&pageno=' + pg + '&callback='
        let videos = [];
        let data = await this.request(url);
        data = data.data;
        videos = _.map(data.movies, (it) => {
            let pic = it.cover.startsWith("//") ? 'http:' + it.cover : it.cover;
            return {
                vod_id: tid + '*' + it.id,
                vod_name: it.title,
                vod_pic: pic,
                vod_remarks: it.comment,
            }
        });
    ...
    async detail(inReq, _outResp) {
        ...
        for (const id of ids) {
            ...
            let url = 'https://api.web.360kan.com/v1/detail?cat=' + idtemp[0] + '&id=' + idtemp[1];
            let data = await this.request(url);
            data = data.data;
            let vod = {
                vod_id: id,
                vod_name: data.title,
                // vod_pic: ,
                vod_remarks: data.description,
                type_name: data.moviecategory[0],
            ...
            const linksDict = data.playlinksdetail;
            for (const key in linksDict) {
                if (Object.hasOwnProperty.call(linksDict, key)) {
                    const value = linksDict[key];
                    ...
                    // for (const play of data.url_content.split('\n')) {
                        nameUrls.push(value.id + '$' + value.default_url.replace(/\?.*$/, ''));
                    // }
    ...
    async play(inReq, _outResp) {
        ...
        const sniffer = await inReq.server.messageToDart({
            ...
            opt: {
                ...
                url: 'https://jx.jsonplayer.com/player/?url=' + tid,
                timeout: 5000,
