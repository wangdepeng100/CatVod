...
async function home(_inReq, _outResp) {
    const html = await request(`${url}/vodshow/1-----------.html`);
    const $ = load(html);
    return {
...
async function category(inReq, _outResp) {
    ...
    if (tid.startsWith('s-')) {
        ...
        const html = await request(`${url}/vodsearch/${href.join('-')}.html`);
        const $ = load(html);
        ...
        if (page == 1 && Object.keys(extend).length == 0) {
            const html = await request(`${url}/vodshow/${tid}-----------.html`);
            $ = load(html);
        ...
        if ($ === null) {
            ...
            const html = await request(`${url}/vodshow/${href.join('-')}.html`);
            $ = load(html);
        }
...
async function search(inReq, _outResp) {
    ...
    let page = pg || 1;
    if (page == 0) page = 1;
    const html = await request(`${url}/vodsearch/-------------.html?wd=${wd}`);
    const $ = load(html);
    const videos = $('div.module-items > div.module-search-item')
        .map((_, div) => {
            const t = $(div).find('div.video-info-header h3 a')[0];
